...
- 更早的同事版本不具备自我更新能力，必须先人工完整安装一次明确标有 `first_install` 的首次安装包；从 `101` 开始，GUI 才会在启动时检查 GitHub Release。
- 检测到新版本后，标题栏会显示更新按钮；下载完成后点击“更新”，程序会安装并重启。
- 在线更新包只替换程序文件，不覆盖同事电脑上的项目配置、Excel 路径、账号密码、日志、报告和浏览器数据。
- `baidu_data_automation_update_*.zip` 是增量更新包，缺少 `configs/`，不能解压到新电脑后直接运行；新电脑必须使用 `baidu_data_automation_first_install_*.zip`。
...
GUI 使用 PyInstaller `onefile + windowed`，发布 ZIP 将 `百度数据自动化控制台.exe` 放在根目录。首次安装包包含默认项目配置，在线更新包不包含任何用户配置；两类包都排除本机 `secrets.json`，账号和 OAuth 授权只通过独立 `.baidu-secrets` 配置包在公司内部传递。
