...
<p align="center">
  <img src="https://img.shields.io/badge/正式项目-9个-success" alt="9 production projects">
  <img src="https://img.shields.io/badge/百度OAuth授权-11路-success" alt="11 OAuth grants">
  <img src="https://img.shields.io/badge/源码规模-~110K行-informational" alt="~110K lines">
  <img src="https://img.shields.io/badge/测试规模-~15K行-informational" alt="~15K test lines">
</p>
...
- [百度 API 模式](#-百度-api-模式)
- [配置体系](#-配置体系)
- [Excel 写入安全](#-excel-写入安全)
- [Chrome 自动化策略](#-chrome-自动化策略)
- [在线更新](#-在线更新)
- [开发指南](#-开发指南)
- [构建与发布](#-构建与发布)
- [版本历史](#-版本历史)
...
| 🔵 登录快商通客户端，手动导出 Excel/CSV | 直接解析同事导出到 `kst_exports/` 的文件，表头动态识别 |
| 🔵 逐行对照两个数据源，手工 VLOOKUP 合并 | `data_merger.py` 按账户+时段自动对齐合并，未知行输出报告 |
| 🔵 找到 Excel 对应 sheet/区域/格子逐格粘贴 | `excel_inspector.py` 扫描表头→`excel_writer.py` 定位写入→自检复核 |
...
```
              ...
              │                          │                          │
    ┌─────────▼─────────┐    ┌──────────▼──────────┐    ┌──────────▼──────────┐
    │   百度数据采集层     │    │   快商通数据解析层     │    │    Excel 读写层      │
    │                    │    │                      │    │                      │
    │ • baidu_auto       │    │ • kst_export_parser   │    │ • excel_inspector    │
    │ • baidu_daily      │    │ • kst_daily_parser    │    │ • excel_writer       │
    │ • baidu_parser     │    │ • kst_parser (口径)    │    │ • excel_engine       │
    │ • baidu_api_*.py   │    │                      │    │ • daily_excel_*      │
    │ • baidu_browser     │    └──────────────────────┘    └─────────────────────┘
    │ • baidu_session     │
    │ • baidu_multi_source│
    │ • baidu_validator   │
...
<table>
<tr>
<th>类别</th><th>技术选型</th><th>说明</th>
</tr>
<tr>
<td>语言</td><td>Python 3.11+</td><td>64-bit，项目自包含 runtime，不污染系统 PATH</td>
</tr>
<tr>
<td>桌面 GUI</td><td>PySide6 + Qt 6</td><td>项目切换、任务执行、控制台日志、系统托盘</td>
</tr>
<tr>
<td>终端 UI</td><td>Rich</td><td>交互式菜单、彩色表格、状态 banner</td>
</tr>
<tr>
<td>浏览器自动化</td><td>Playwright (Chrome CDP)</td><td>仅连接/启动 Google Chrome 调试端口 9222</td>
</tr>
<tr>
<td>百度 API</td><td>OAuth 2.0 + HMAC</td><td>服务商应用 openBD，Token 自动刷新</td>
</tr>
<tr>
<td>Excel 读写</td><td>openpyxl + xlrd</td><td>不重建工作簿，表头驱动定位写入</td>
</tr>
<tr>
<td>打包</td><td>PyInstaller + Inno Setup</td><td>onefile windowed EXE + 完整 Windows 安装器</td>
</tr>
<tr>
<td>更新分发</td><td>GitHub Releases API</td><td>GUI 自动检测 → 下载 → 静默安装 → 重启</td>
</tr>
<tr>
<td>CLI 路由</td><td>argparse</td><td>30+ 独立 --mode 入口，统一路由到模块函数</td>
</tr>
<tr>
<td>测试</td><td>pytest</td><td>覆盖配置、Excel、百度、快商通、合并、编排、GUI、发布包</td>
</tr>
</table>
...
:: 下载百度数据 + 解析快商通文件 + 合并 + 写入 Excel（一站式）
...
:: 分阶段调试
.venv\Scripts\python.exe main.py --mode inspect-excel         # 1. 扫描 Excel 结构
.venv\Scripts\python.exe main.py --mode fetch-baidu-auto --period 15点  # 2. 拉百度数据
.venv\Scripts\python.exe main.py --mode parse-kst-export --period 15点   # 3. 解析快商通
.venv\Scripts\python.exe main.py --mode merge-data             # 4. 合并
.venv\Scripts\python.exe main.py --mode write-excel --period 15点        # 5. 写入 Excel
...
┌─────────────────┐     ┌──────────────────┐
│  百度营销后台     │     │  快商通人工导出文件  │
│  (API / Chrome)  │     │  (kst_exports/)   │
└────────┬────────┘     └────────┬─────────┘
         │                       │
         ▼                       ▼
┌─────────────────┐     ┌──────────────────┐
│  baidu_auto.py   │     │ kst_export_parser │
│  baidu_daily.py  │     │ kst_daily_parser  │
│  baidu_api_*.py  │     │                   │
└────────┬────────┘     └────────┬─────────┘
         │                       │
         ▼                       ▼
