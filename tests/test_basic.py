...
def test_textual_dependency_is_declared_in_existing_requirements_file():
    root = Path(__file__).resolve().parents[1]
...
def test_textual_workbench_entry_files_exist_and_keep_cmd_menu_separate():
    root = Path(__file__).resolve().parents[1]

    assert (root / "textual_app.py").exists()
    assert (root / "run_textual.bat").exists()
    assert (root / "docs" / "textual_tui_sop.md").exists()
    stable_menu = (root / "run_menu.bat").read_text(encoding="utf-8")
    assert "textual_app.py" not in stable_menu
    assert "menu.py" in stable_menu


def test_run_textual_bat_sets_utf8_and_starts_independent_workbench():
    root = Path(__file__).resolve().parents[1]
    script = (root / "run_textual.bat").read_text(encoding="utf-8")

    assert "cd /d D:\\自动化脚本\\hourly_report_bot_release_v0.4.4" in script
    assert "chcp 65001" in script
    assert "PYTHONUTF8=1" in script
    assert "PYTHONIOENCODING=utf-8" in script
    assert ".venv\\Scripts\\python.exe textual_app.py" in script


def test_textual_app_wires_existing_commands_without_password_or_recommendation_logic():
    root = Path(__file__).resolve().parents[1]
    content = (root / "textual_app.py").read_text(encoding="utf-8")
    ...
    assert "secrets/secrets.json" not in content
    assert "password" not in content.lower()
    assert "推荐下一步操作" not in content
    assert "推荐运行" not in content
    assert "create_subprocess" in content
    assert "已有任务运行中" in content
    assert 'completion_report="final_run_report.json"' in content
    assert 'completion_report="daily_final_run_report.json"' in content


def test_textual_sop_documents_startup_capabilities_and_security_boundary():
    root = Path(__file__).resolve().parents[1]
    content = (root / "docs" / "textual_tui_sop.md").read_text(encoding="utf-8")

    assert "run_textual.bat" in content
    for text in ["运行日报", "运行小时报", "预检", "凭据检查", "doctor", "reports", "logs"]:
        assert text in content
    for text in ["不保存密码", "不显示密码", "不替代 run_menu.bat", "不重写业务逻辑"]:
        assert text in content


def test_textual_successful_hourly_report_marks_completed_period(tmp_path):
    import json

    from modules.task_status import is_hourly_done
    from textual_app import mark_completed_report

    report_dir = tmp_path / "reports"
    report_dir.mkdir()
    (report_dir / "final_run_report.json").write_text(
        json.dumps({
            "project_id": "proj",
            "passed": True,
            "write_summary": {"write_count": 1, "verification_passed": True},
        }, ensure_ascii=False),
        encoding="utf-8",
    )

    assert mark_completed_report(tmp_path, "final_run_report.json", "11点") is True
    assert is_hourly_done(tmp_path, "proj", "11点") is True


def test_textual_zero_write_daily_report_does_not_mark_completion(tmp_path):
    import json

    from modules.task_status import is_daily_done
    from textual_app import mark_completed_report

    report_dir = tmp_path / "reports"
    report_dir.mkdir()
    (report_dir / "daily_final_run_report.json").write_text(
        json.dumps({
            "project_id": "proj",
            "passed": True,
            "write_summary": {"write_count": 0, "verification_passed": True},
        }, ensure_ascii=False),
        encoding="utf-8",
    )

    assert mark_completed_report(tmp_path, "daily_final_run_report.json") is False
    assert is_daily_done(tmp_path, "proj") is False
