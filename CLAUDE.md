...
测试文件 `tests/test_basic.py` 包含约 200 个测试，覆盖：项目配置、Excel 读写、百度解析、快商通解析、数据合并、一键流编排、浏览器设置、Chrome 调试启动、preflight、桌面 GUI、菜单路由、控制台 UI、任务状态等。
...
BAT 会固定工作目录、UTF-8 环境和 `.venv` Python，先跑 preflight，失败则停止。
