...
- 当前版本：`v1.0 内部发布版（Rich 控制台收口、沈阳双百度来源、自动安装修复与商务通表头兼容）`
...
8. 人工菜单已收口为 Rich 控制台首页，OpenClaw 固定入口不变。
   - 首页入口为 `1. 小时报`、`2. 日报`、`3. 切换项目`、`4. 检查条件项`、`5. 更多功能`，`R. 刷新状态` 仅用于刷新首页显示。
   - 菜单布局调整不影响 OpenClaw 固定入口；自动执行仍优先调用 `run_openclaw_hourly.bat` 与 `run_openclaw_daily.bat`。
   - OpenClaw 不需要按菜单层级点击执行小时报或日报，避免将人工界面文案变化误认为命令入口变化。
...
说明：人工操作时用 Rich 控制台菜单入口；首页主入口为 `1. 小时报`、`2. 日报`、`3. 切换项目`、`4. 检查条件项`、`5. 更多功能`。首次运行或依赖安装不完整时，该入口会自动安装/修复环境后再打开菜单。AI / 夏思道自动执行时优先使用下方固定批处理入口，不依赖菜单布局。
...
| 首次双击菜单提示缺少 `openpyxl` | 旧包的 `run_menu.bat` 未自动安装，或上次安装中断 | 使用 `v1.0` 内部发布包重新解压后双击 `run_menu.bat`；新包会自动安装或修复环境 |
| 安装时提示找不到 `xlwings` | `v0.4.19` 旧包把 Excel COM 备用依赖当成默认必装项 | 使用 `v1.0` 内部发布包；当前 `openpyxl` 项目默认安装不再安装 `xlwings` |
...
`v1.0 内部发布版（Rich 控制台收口、沈阳双百度来源、自动安装修复与商务通表头兼容）`
...
- 南京牛日期 / 时段全局字段定位
- 夏思道命令行 SOP
- Rich 控制台首页收口，人工菜单主入口简洁化，OpenClaw 固定批处理入口保持不变
