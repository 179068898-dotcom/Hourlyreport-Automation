...
`2026.7.22.108` 是当前标准安装器基线。新电脑只分发 `Hourlyreport_automation_setup_v2026.7.22.108.exe`，用户选择目录后一次安装完成。
...
```cmd
.venv\Scripts\python.exe tools\build_windows_installer.py --version 2026.7.22.108
```

7. 生成只含程序文件的在线更新包：

```cmd
.venv\Scripts\python.exe tools\build_release.py --online-update --version 2026.7.22.108
```

8. GitHub Release tag 使用 `v2026.7.22.108`，在线更新资产名为 `Hourlyreport_automation_v2026.7.22.108.zip`；完整安装器名为 `Hourlyreport_automation_setup_v2026.7.22.108.exe`，两者用途不同。
