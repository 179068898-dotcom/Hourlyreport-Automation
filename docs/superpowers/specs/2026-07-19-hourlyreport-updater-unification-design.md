...
`2026.7.19.104` 重新制作完整内部包并作为统一基线，由管理员直接分发给全部同事。该基线已经使用新仓库、新资产名和新 EXE 名；104 及后续版本只访问 `Hourlyreport-Automation`，不实现旧仓库增量迁移。

## EXE 兼容

更新包只包含新的 `hourlyreport_automation.exe`，程序重启和快捷方式均指向该唯一主入口，不生成旧中文文件名副本。
...
产品标题、主要模块和核心操作使用 Microsoft YaHei Bold；“系统配置”“数据模式”、二级信息和小标题使用常规 Microsoft YaHei。两个菜单按钮的左右内边距和固定宽度同时缩减，布局间距从 5 px 缩减到 2 px，使文字视觉间隔约减半，悬浮底色仍完整包裹文字。
